...
export default class StacEndpoint {
  ...
  private get root(): Promise<StacRootDocument> {
    if (!this.root_) {
      this.root_ = fetchStacDocument<StacRootDocument>(this.baseUrl).catch((e) => {
  ...
  async getCollection(collectionId: string): Promise<StacCollection> {
    ...
    const collectionUrl = new URL(collectionId, collectionsUrl + '/').toString();
  ...
  async getCollectionItems(
    ...
    return itemsDoc.features.map((item) => parseStacItem(item as unknown as StacDocument));
  }
  ...
  async getCollectionItem(
    ...
    const itemsUrl = getLinkUrl(collection as unknown as StacDocument, 'items', this.baseUrl);
    if (!itemsUrl) {
      throw new EndpointError(
