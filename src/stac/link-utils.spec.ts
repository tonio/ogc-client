...
describe('stac/link-utils', () => {
  ...
  describe('fetchStacDocument', () => {
    it('fetches and parses JSON document successfully', async () => {
      ...
      const result = await linkUtils.fetchStacDocument('https://example.com/api');
  ...
  describe('getLinkUrl', () => {
    ...
    it('returns null when link is not found', () => {
      ...
      const url = linkUtils.getLinkUrl(doc, 'nonexistent', 'https://example.com/');
      expect(url).toBeNull();
    });
  ...
  describe('assertHasLinks', () => {
    it('does not throw when all required links are present', () => {
      ...
      expect(() => linkUtils.assertHasLinks(doc, ['self', 'root'])).not.toThrow();
    });
    ...
    it('throws EndpointError when some required links are missing', () => {
      ...
      expect(() => linkUtils.assertHasLinks(doc, ['self', 'root', 'items'])).toThrow(
        EndpointError
      );
      expect(() => linkUtils.assertHasLinks(doc, ['self', 'root', 'items'])).toThrow(
        'missing required links with rel types: root, items'
      );
    });
  });
});
