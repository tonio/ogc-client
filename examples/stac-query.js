  ...
  try {
    ...
    if (collections.length > 0) {
      ...
      if (collection.keywords && collection.keywords.length > 0) {
        console.log(`   Keywords: ${collection.keywords.slice(0, 5).join(', ')}`);
      }
      ...
      if (collection.extent.temporal.interval.length > 0) {
        ...
        console.log(`   Temporal Extent: ${interval[0] || 'open'} to ${interval[1] || 'open'}`);
      }

      // Assets
      if (collection.assets) {
        const assetKeys = Object.keys(collection.assets);
        console.log(`   Collection Assets: ${assetKeys.length} (${assetKeys.slice(0, 3).join(', ')}${assetKeys.length > 3 ? '...' : ''})`);
      }
      ...
      items.forEach((item, idx) => {
        ...
        console.log(`      Assets: ${assetCount} (${assetKeys}${assetCount > 3 ? '...' : ''})`);
        console.log('');
      });
      ...
      if (items.length > 0 && items[0].bbox) {
        ...
        console.log(`   Using bbox from first item (expanded by ${expansion}°)`);
        console.log(`   BBox: [${filterBbox.map(v => v.toFixed(2)).join(', ')}]`);
        console.log(`   Limit: 5\n`);
        ...
        filteredItems.forEach((item, idx) => {
          console.log(`   ${idx + 1}. ${item.id} - ${item.properties.datetime || 'N/A'}`);
        });
        console.log('');
      } else {
      ...
      if (items.length > 0) {
        ...
        Object.entries(singleItem.assets).slice(0, 5).forEach(([key, asset]) => {
          console.log(`     • ${key}:`);
